...
import { encryptWithPassword, arrayBufferToBase64 } from "../crypto.js"
...
async function createPaste(env, content, isPrivate, expire, short, createDate, passwd, viewPasswd, existingMeta, asMarkdown) {
  ...
  // Handle optional view protection (encrypt content with AES-GCM using provided password)
  let contentToStore = content
  if (viewPasswd && viewPasswd.length > 0) {
    const { cipher, salt, iv } = await encryptWithPassword(content, viewPasswd)
    options.metadata.vProtected = true
    options.metadata.vSalt = arrayBufferToBase64(salt)
    options.metadata.vIv = arrayBufferToBase64(iv)
    contentToStore = cipher
  } else if (existingMeta && existingMeta.vProtected) {
    // If previously protected but no new view password provided, reject to avoid losing protection inadvertently
    throw new WorkerError(400, "view password required to update protected paste")
  } else {
    options.metadata.vProtected = false
  }
...
export async function handlePostOrPut(request, env, ctx, isPut) {
  ...
  // const filename = form.get("c") && getDispFilename(form.get("c").fields) // removed
